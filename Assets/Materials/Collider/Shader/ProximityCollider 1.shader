Shader "Custom/ProximityCollider" {
	Properties {
		...
		_PlayerPosition ("Player Position", vector) = (0,0,0,0) // The location of the player - will be set by script
      	...
      	[HDR] _OutlineColour ("Outline Colour", color) = (1.0,1.0,0.0,1.0) // Colour of the outline
	}
	SubShader {
		...
		float4 frag(vertexOutput input) : COLOR 
		{
            ...
            if (dist < _VisibleDistance) {
               return tex2D(_MainTex, float4(input.tex)) * _Color; // Visible
            }
